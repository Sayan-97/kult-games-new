...
function GameSlide({ item, index }: { item: typeof games[0]; index: number }) {
  ...
  return (
    <div
      className="flex flex-col md:flex-row items-stretch gap-4 md:gap-8 relative group"
    ...
    >
      {/* CSS spotlight — no framer-motion MotionValues; works correctly on embla clone slides */}
      <div
        ...
        style={{
          background:
            "radial-gradient(600px circle at var(--mx) var(--my), rgba(84, 22, 233, 0.15), transparent 80%)",
        }}
      />
      ...
      <div className="md:w-2/3">
        <Link
          ...
          onClick={() =>
            trackClarityEvent(
              `game_card_click:${item.name.toLowerCase().replaceAll(" ", "_")}`
            )
          }
          className="relative h-[279px] md:h-[579px] rounded-3xl overflow-hidden block cursor-pointer hover:opacity-90 transition-opacity border border-white/5 group-hover:border-white/10 duration-500"
        >
          <Image
            src={item.image}
            alt={item.name}
          ...
          <div className="absolute inset-0 bg-[radial-gradient(217.24%_242.87%_at_217.75%_107.58%,rgba(127,139,210,0.30)0%,rgba(89,106,197,0.00)100%)] p-6 flex flex-col justify-between items-start">
            <div className="space-y-2">
              ...
              <Image
                ...
                alt="platform"
                draggable={false}
                width={item.platWidth}
                height={item.platHeight}
              />
            </div>
    ...
    >
      ...
      <div className="container space-y-12">
        <AnimatedSection threshold={0.5}>
          <h2 className="font-ethnocentric">Explore Intelligent Experiences</h2>
        </AnimatedSection>
        <Carousel
          opts={{ loop: true }}
          ...
          <CarouselContent className="-ml-8">
            {games.map((item, index) => (
              <CarouselItem key={index} className="pl-8">
                <GameSlide item={item} index={index} />
              </CarouselItem>
            ))}
          </CarouselContent>
          <CarouselPrevious
            className="flex -left-4 md:-left-12 bg-[#191934] border-[#5F33D6] hover:bg-[#191934]/80"
            onClick={() => trackClarityEvent("games_carousel_previous")}
          />
          <CarouselNext
            className="flex -right-4 md:-right-12 bg-[#191934] border-[#5F33D6] hover:bg-[#191934]/80"
